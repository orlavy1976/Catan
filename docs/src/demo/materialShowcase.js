...
/**
 * Add Material showcase to existing game
 */
export function addMaterialShowcaseToGame(app, root) {
  // Create toggle button in top-right corner
  const toggleButton = createMaterialButton('Material Demo', {
    variant: 'outlined',
    size: 'small',
  });
  
  toggleButton.container.x = app.screen.width - toggleButton.width - 20;
  toggleButton.container.y = 20;
  
  let showcaseOpen = false;
  let currentShowcase = null;
  
  toggleButton.onClick(() => {
    console.log('Toggle button clicked, showcaseOpen:', showcaseOpen);
    
    if (showcaseOpen && currentShowcase) {
      // Close existing showcase
      console.log('Closing showcase');
      animateFade(currentShowcase, 0, MATERIAL_MOTION.duration.fast).then(() => {
        if (root.children.includes(currentShowcase)) {
          root.removeChild(currentShowcase);
        }
        currentShowcase = null;
        showcaseOpen = false;
        toggleButton.setText('Material Demo');
      }).catch(err => {
        console.error('Error closing showcase:', err);
      });
    } else {
      // Open new showcase
      console.log('Opening showcase');
      try {
        currentShowcase = createMaterialShowcase(app, root);
        showcaseOpen = true;
        toggleButton.setText('Close Demo');
      } catch (err) {
  ...
  root.addChild(toggleButton.container);
